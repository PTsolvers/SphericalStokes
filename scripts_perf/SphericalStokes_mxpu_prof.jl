...
const nr        = haskey(ENV, "NR"     )   ? parse(Int , ENV["NR"]       ) : 16*32 - 1
const nθ        = haskey(ENV, "NTH"    )   ? parse(Int , ENV["NTH"]      ) : 16*32 - 1
const nφ        = haskey(ENV, "NPH"    )   ? parse(Int , ENV["NPH"]      ) : 16*32 - 1
...
@views function SphericalStokes()
    ...
    maxiter  = 3e1 #1e3                   # maximum number of pseudo-transient iterations
    ...
    MPI.Barrier(MPI.COMM_WORLD) # uncomment for prof
